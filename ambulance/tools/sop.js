...
// - Create SOP module from CPG Wizard pattern.
// - Load sop_index.json and open sop pdf file by page/search.
// - Accept SOP-specific config keys for helper and viewer URLs.
// - Only restore query hash state when the active tool is sop.
// - Use live search while typing and dismiss keyboard from Enter/result scrolling.
// - Remove result accuracy percentages.
// - Do not persist the search query after leaving and reopening SOP.
...
// SOP:
// - Smart search over sop_index.json
// - Shows matching SOP results as the user types
// - Buttons:
//   1) Open SOP (page 1)
//   2) Search SOP (open PDF viewer with #search=<term>)
//   3) Open SOP at page (user enters page number)
...
export async function run(root) {
  ...
  // =========================
  const FALLBACK = {
    urlIndex: "https://docs.niwashibase.com/helpers/sop_index.json",
    urlPageBase: "https://docs.niwashibase.com/viewer/web/?file=/docs/sop-101qq9f2w.pdf#page=",
    urlSearchBase: "https://docs.niwashibase.com/viewer/web/?file=/docs/sop-101qq9f2w.pdf#search="
  };
  ...
  function getCfg() {
    ...
    const urlIndex = (cfg && (cfg.urlSopIndex || cfg.urlSopIndexJson || cfg.urlSopindexJson)) ? String(cfg.urlSopIndex || cfg.urlSopIndexJson || cfg.urlSopindexJson) : FALLBACK.urlIndex;
    const urlPage = (cfg && cfg.urlSopPage) ? String(cfg.urlSopPage) : FALLBACK.urlPageBase;
    const urlKeyword = (cfg && cfg.urlSopKeyword) ? String(cfg.urlSopKeyword) : FALLBACK.urlSearchBase;
  ...
  function ensureViewerModal() {
    ...
    modal.innerHTML = `
      <div class="pv-sheet" role="dialog" aria-modal="true">
        <div class="pv-bar">
          ...
          <div class="pv-title" id="pvTitle">SOP</div>
        </div>
    ...
    function buildOverlayState(overlayToken) {
      const baseState = (history.state && history.state.tool) ? history.state : { tool: "sop", params: {} };
  ...
  root.insertAdjacentHTML("afterbegin", `
    <div class="cw-wrap">
      <div class="cw-title-row">
        <h2 class="cw-title">SOP</h2>
        <div class="cw-version">SOP v4.4 2024</div>
      </div>
      ...
      <div class="cw-panel">
        ...
        <div class="cw-actions">
          <button class="cw-btn" id="cwOpenCpg" type="button">Open SOP</button>
  ...
  function openAtPage(page, title) {
    ...
    modal.__open(CFG.urlPageBase + String(page), title || "SOP", `page:${page}`);
  }
  ...
  // =========================
  // LOAD INDEX (sop_index.json)
  // =========================
  async function loadIndex() {
    const res = await fetch(CFG.urlIndex, { cache: "no-store" });
    if (!res.ok) throw new Error(`Failed to load sop_index.json (${res.status})`);
    const data = await res.json();
    const items = Array.isArray(data) ? data : (Array.isArray(data.items) ? data.items : null);
    if (!Array.isArray(items)) throw new Error("sop_index.json has no items array.");
    return items;
  }
  ...
  // =========================
  function buildSearcher(sop_items) {
    if (!Array.isArray(sop_items)) throw new Error("sop_items must be an array.");
    ...
    const PREPARED_ITEMS = sop_items.map((item) => {
    ...
    function search(user_query, search_options) {
      ...
      for (let i = 0; i < PREPARED_ITEMS.length; i++) {
        ...
        results.push({
          ...
          sop_number: entry.item.sop_number,
          sop_section: entry.item.sop_section,
        ...
        if (qTokens.length <= 1) {
          ...
          if (bestItem && bestDist <= 2) {
            suggestion = {
              ...
              sop_number: bestItem.sop_number,
              sop_section: bestItem.sop_section,
              type: bestItem.type,
              distance: bestDist,
      ...
      const outResults = results.slice(0, MAX_RESULTS).map((r) => {
        return {
          id: r.id,
          title: r.title,
  ...
  function renderResults(payload) {
    ...
    if (res.length) {
      ...
      res.forEach((x) => {
        ...
        const sopNumber = x.sop_number ? `SOP ${x.sop_number}` : "SOP";
        const sopSection = x.sop_section || "";

        card.innerHTML = `
          <div class="left">
            <div class="name">${escapeHtml(x.title || "")}</div>
            <div class="sub">
              ${sopSection ? `<div class="cw-sop-section">${escapeHtml(sopSection)}</div>` : ``}
              ${range ? `<div class="cw-sop-section">Page ${escapeHtml(range)}</div>` : ``}
            </div>
          </div>

          <div class="cw-right">
            <div class="cw-badge">${escapeHtml(sopNumber)}</div>
          </div>
        `;
        ...
        function open() {
          ...
          openAtPage(Number.isFinite(p) && p > 0 ? p : 1, x.title || "SOP");
        }
    ...
    if (sug) {
      ...
      const sopNumber = sug.sop_number ? `SOP ${sug.sop_number}` : "SOP";
      const sopSection = sug.sop_section || "";
      ...
      function open() {
        ...
        openAtPage(Number.isFinite(p) && p > 0 ? p : 1, sug.title || "SOP");
      }
    ...
    $resTitle.textContent = "Results";
    $resMeta.textContent = "—";
    $list.innerHTML = `<div class="cw-empty">Type to search SOP.</div>`;
  }
  ...
  $searchCpg.addEventListener("click", () => {
    safeVibrate(6);
    const term = promptText("Search SOP", $query.value || "");
    if (!term) return;
  ...
  $openPage.addEventListener("click", () => {
    safeVibrate(6);
    const n = promptNumber("Open SOP at page number", "1");
    if (!n) return;
    openAtPage(n, `Page ${n}`);
  });
